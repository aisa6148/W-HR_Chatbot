...
class ChatLogger {
   ...
   */
  constructor(url: String, key: String, botID: String, botName: String, env = 'DEV') {
    const _this = this;
    _this.botID = botID;
    ...
    _this.database
      ...
      .then(() =>
        _this.database.getCollection(constutil.database[env], constutil.collection['User'])
      )
      .then(() =>
        _this.database.getCollection(constutil.database[env], constutil.collection['Usage'])
      )
      .then(() =>
        _this.database.getCollection(
          constutil.database[env],
          constutil.collection['ConversationDetails']
        )
      )
      .then(() =>
        _this.database.getCollection(constutil.database[env], constutil.collection['Message'])
      )
      .catch((error: Error) => logger.error({ location: 'ChatLogger ctor', error }));
  }
   ...
   */
  public async saveMessageDocument(metadata: {
    from: String;
    message: String;
    messageType: String;
    ...
    if (metadata.messageID) document.messageID = metadata.messageID;
    if (metadata.userQuestion) document.userQuestion = metadata.userQuestion;
    if (metadata.luisIntent) document.luisIntent = metadata.luisIntent;
    if (metadata.luisScore) document.luisScore = metadata.luisScore;
    if (metadata.qnaScore) document.qnaScore = metadata.qnaScore;
    if (metadata.qnaResponse) document.qnaResponse = metadata.qnaResponse;
    if (metadata.feedback) document.feedback = metadata.feedback;
    if (metadata.customData) document.customData = metadata.customData;
    ...
    // } else {
    try {
      const result = await this.database.docCreate(
        this.databaseName,
        constutil.collection['Message'],
        document
      );
      return result.id;
    } catch (e) {
  ...
  queryUser(userID: string) {
    ...
    const query =
      "SELECT * FROM r WHERE r.userID='" + userID + "' and r.botID='" + this.botID + "'";
   ...
   */
  queryConversation(conversationID: string) {
    const query =
      "SELECT * FROM r WHERE r.conversationID='" +
      conversationID +
      "' and r.botID='" +
      this.botID +
      "'";
    return this.database.queryCollection(
      this.databaseName,
      constutil.collection['ConversationDetails'],
      query
    );
   ...
   * @param {function(err)} callback executes after completion, contains error as the parameter to callback
   */
  public saveConversationDocument(
    metadata: {
      userID: string;
      conversationID: string;
      channel: string;
   ...
   * @param {function(err)} callback executes after completion, contains error as the parameter to callback
   */
  public updateFeedback(id: string, feedback: string, callback: Function) {
